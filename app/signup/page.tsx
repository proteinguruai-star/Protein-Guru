...
type StepId = 'phone'|'name'|'email'|'age'|'sex'|'weight'|'lifestyle'|'diet'|'finish';
...
export default function SignupWizard() {
  ...
  const order: StepId[] = ['phone','name','email','age','sex','weight','lifestyle','diet','finish'];
  const idx = order.indexOf(step);
  const goNext = () => idx < order.length-1 && setStep(order[idx+1]);
  const goBack = () => idx > 0 && setStep(order[idx-1]);

  // protein lane
  const protein = (w:number) => {
    const min = Math.round(w*0.8);
    const max = Math.round(w*(lifestyle==='gym'?2.0:1.2));
    return {min,max};
  };
  const pr = protein(weight);

  async function submitAll(){
    setSubmitting(true); setServerOk(null);
    try{
      const res = await fetch('/api/signup', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({
          phone: `+91 ${digits10(phone)}`, name, email, age, sex, weight, lifestyle, diet
        })
      });
  ...
  // ---------- shared shell ----------
  const Shell: React.FC<{icon:React.ReactNode; title:React.ReactNode; sub?:React.ReactNode; children?:React.ReactNode;}> =
    ...
    <div className="relative mx-auto w-full max-w-[44rem] rounded-[28px] border border-[var(--ink)]/10 bg-white p-6 md:p-8 shadow-sm">
      ...
      <div className="mt-8 flex items-center justify-between">
        <button onClick={goBack} disabled={idx===0}
          className="inline-flex h-12 w-12 items-center justify-center rounded-full border border-[var(--ink)]/15 text-[var(--ink)]/70 disabled:opacity-40"><ArrowLeft size={18}/></button>
        {step!=='finish' ? (
          <button onClick={goNext} disabled={!valid[step as keyof typeof valid]}
            className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-[var(--ink)] text-[var(--paper)] hover:bg-[var(--green)] transition disabled:bg-[var(--ink)]/40"><ArrowRight size={18}/></button>
        ) : (
          <button onClick={submitAll} disabled={submitting}
            className="inline-flex items-center gap-2 rounded-full bg-[var(--ink)] px-6 py-3 text-[var(--paper)] hover:bg-[var(--green)] transition">
            {submitting?'Saving…':'Finish'} <ArrowRight size={18}/>
          </button>
        )}
      </div>
    </div>
  ...
  return (
    <main className="min-h-screen bg-[var(--paper)] text-[var(--ink)] px-4 py-8 md:py-12" style={style as any}>
      <div className="mx-auto mb-5 max-w-[44rem]"><a href="/" className="text-sm text-[var(--ink)]/70 hover:text-[var(--ink)]">← Back to home</a></div>

      <AnimatePresence mode="wait">
        {/* PHONE */}
        {step==='phone' && (
          <motion.section key="phone" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}}>
            <Shell icon={<Phone size={18}/>} title="What's your phone number?" sub="We’ll send updates and your invite link here.">
          ...
          <motion.section key="name" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}}>
            <Shell icon={<User size={18}/>} title="What's your name?" sub="First name is required; last name optional.">
              <input
                ...
                placeholder="First Last" value={name} onChange={e=>setName(e.target.value)} />
            </Shell>
          </motion.section>
        )}
        ...
        {step==='sex' && (
          <motion.section key="sex" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}}>
            <Shell icon={<Shield size={18}/>} title="How do you identify?">
              <Segment value={sex} onChange={v=>setSex(v as Sex)} items={[
        ...
        {step==='lifestyle' && (
          <motion.section key="lifestyle" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}}>
            <Shell icon={<Dumbbell size={18}/>} title="Lifestyle">
              <Segment value={lifestyle} onChange={v=>setLifestyle(v as Lifestyle)} items={[
        ...
        {step==='diet' && (
          <motion.section key="diet" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}}>
            <Shell icon={<Leaf size={18}/>} title="What’s your diet?" sub="Tap one — you can change this later.">
              <Segment value={diet} onChange={v=>setDiet(v as Diet)} items={[
          ...
          <motion.section key="finish" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}}>
            <div className="mx-auto w-full max-w-[44rem] rounded-[28px] border border-[var(--ink)]/10 bg-white p-6 md:p-8 shadow-sm">
              ...
              <div className="mt-6 rounded-2xl border border-[var(--ink)]/10 bg-[var(--paper)] p-5">
                <div className="text-3xl md:text-4xl font-semibold text-[var(--ink)]">{pr.min}–{pr.max} g / day</div>
                <div className="mt-3 h-2 w-full rounded-full bg-[var(--ink)]/10">
                  <motion.div
                    className="h-2 rounded-full bg-[var(--green)]"
                    initial={{ width: 0 }}
                    animate={{ width: `${Math.min(100, (pr.max/(2.2*weight))*100)}%` }}
              ...
              <div className="mt-6 flex flex-wrap items-center gap-3 text-sm">
                <span className="inline-flex items-center gap-2 rounded-full border border-[var(--ink)]/15 px-3 py-1"><CheckCircle size={16}/> Veg/Egg/Non-veg ready</span>
                <span className="inline-flex items-center gap-2 rounded-full border border-[var(--ink)]/15 px-3 py-1"><CheckCircle size={16}/> WhatsApp & Blinkit lists</span>
              </div>

              <div className="mt-8 flex items-center justify-between">
                <button onClick={goBack} className="inline-flex h-12 w-12 items-center justify-center rounded-full border border-[var(--ink)]/15 text-[var(--ink)]/70"><ArrowLeft size={18}/></button>
                <button onClick={submitAll} disabled={submitting}
                  className="inline-flex items-center gap-2 rounded-full bg-[var(--ink)] px-6 py-3 text-[var(--paper)] hover:bg-[var(--green)] transition">
                  {submitting?'Saving…':'Finish'} <ArrowRight size={18}/>
                </button>
              ...
              {serverOk===true && (
                <div className="mt-5 rounded-2xl border border-[var(--green)]/40 bg-[var(--green)]/10 p-4 text-[var(--ink)]">
                  🎉 <strong>Thanks for signing up!</strong> Your daily protein lane is <strong>{pr.min}–{pr.max}g</strong>.
                  We’re going live soon — you’ll get an invite the moment we launch, and Protein Guru will help you hit that number every day. 💪
                </div>
              )}
              {serverOk===false && <div className="mt-4 rounded-2xl border border-red-300 bg-red-50 p-3 text-red-700">Oops, couldn’t save right now. Try again in a bit.</div>}
            </div>
          </motion.section>
        )}
      </AnimatePresence>
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex items-center gap-2 rounded-xl border border-[var(--ink)]/15 px-3 py-3">
        🇮🇳 <ChevronDown size={14} className="opacity-60"/><span className="opacity-80">+91</span>
      </div>
      <input
        ref={ref}
        autoFocus
...
function Segment<T extends string>({ value, onChange, items }:{
  value: T|''; onChange: (v:T)=>void; items: {value:T; label:string; icon?:React.ReactNode}[];
}){
  return (
    <div className="grid grid-cols-3 gap-2">
      {items.map(it=>{
        ...
        return (
          <button key={it.value as string} type="button" onClick={()=>onChange(it.value)}
            className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm md:text-base
              ${active?'border-[var(--green)] bg-[var(--green)]/10 text-[var(--ink)]'
...
function Meter({ min, max, step, value, onChange, unit }:{
  min:number; max:number; step:number; value:number; onChange:(n:number)=>void; unit:string;
}){
  const pct = Math.round(((value-min)/(max-min))*100);
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        <span className="text-[var(--ink)]/70 text-sm">{min}{unit}</span>
        <span className="rounded-full bg-[var(--ink)]/5 px-3 py-1 text-[var(--ink)] text-sm font-medium">
          {value}{unit}
        </span>
        <span className="text-[var(--ink)]/70 text-sm">{max}{unit}</span>
      </div>
